...
const FAVORITES_INITIAL_LIMIT: usize = 30;
const FAVORITES_SERVER_SONG_CAP: usize = 300;
const FAVORITES_SERVER_ALBUM_CAP: usize = 250;
const FAVORITES_SERVER_ARTIST_CAP: usize = 200;
...
pub fn FavoritesView() -> Element {
    ...
    let favorites = use_resource(move || {
        ...
        let load_artists = active_tab() == "artists";
        async move {
            ...
            let cache_key = format!(
                "view:favorites:v2:{}:artists={}",
                cache_server_ids.join("|"),
                if load_artists { "1" } else { "0" }
            );
            ...
            for server in servers {
                ...
                if let Ok((a, al, s)) = client.get_starred().await {
                    if load_artists {
                        let mut limited_artists = a;
                        limited_artists.truncate(FAVORITES_SERVER_ARTIST_CAP);
                        artists.extend(limited_artists);
                    }
                    let mut limited_albums = al;
                    limited_albums.truncate(FAVORITES_SERVER_ALBUM_CAP);
                    albums.extend(limited_albums);
                            ...
                            match tab.as_str() {
                                ...
                                "albums" => rsx! {
                                    if albums.is_empty() {
                                        ...
                                        {
                                            let limit = display_limit();
                                            let display: Vec<Album> = albums.iter().take(limit).cloned().collect();
                                            rsx! {
                                                div { class: "space-y-4",
                                                    ...
                                                    if albums.len() > limit {
                                                        div { class: "pt-2 flex justify-center",
                                                            button {
                                                                class: "px-4 py-2 rounded-xl bg-zinc-800/60 hover:bg-zinc-700 text-zinc-300 hover:text-white transition-colors text-sm",
                                                                onclick: move |_| display_limit.set(display_limit().saturating_add(FAVORITES_INITIAL_LIMIT)),
                                                                "Show more ({albums.len() - limit} remaining)"
                                                            }
                                                        }
                                                    }
                                                }
                                ...
                                "artists" => rsx! {
                                    if artists.is_empty() {
                                        ...
                                        {
                                            let limit = display_limit();
                                            let display: Vec<Artist> = artists.iter().take(limit).cloned().collect();
                                            rsx! {
                                                div { class: "space-y-4",
                                                    ...
                                                    if artists.len() > limit {
                                                        div { class: "pt-2 flex justify-center",
                                                            button {
                                                                class: "px-4 py-2 rounded-xl bg-zinc-800/60 hover:bg-zinc-700 text-zinc-300 hover:text-white transition-colors text-sm",
                                                                onclick: move |_| display_limit.set(display_limit().saturating_add(FAVORITES_INITIAL_LIMIT)),
                                                                "Show more ({artists.len() - limit} remaining)"
                                                            }
                                                        }
                                                    }
                                                }
